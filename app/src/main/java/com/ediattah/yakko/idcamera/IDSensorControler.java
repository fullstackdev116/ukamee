 ...
 * Desc	        ${加速度控制器，用来控制对焦}
 */
public class IDSensorControler implements SensorEventListener {
    ...
    private static IDSensorControler mInstance;
    ...
    private IDSensorControler(Context context) {
        mSensorManager = (SensorManager) context.getSystemService(Activity.SENSOR_SERVICE);
        mSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER);// TYPE_GRAVITY
    }

    public static IDSensorControler getInstance(Context context) {
        if (mInstance == null) {
            mInstance = new IDSensorControler(context);
        }
        return mInstance;
    }
