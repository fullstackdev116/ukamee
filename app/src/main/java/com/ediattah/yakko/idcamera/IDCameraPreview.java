 ...
 * Desc	        ${相机预览}
 */
public class IDCameraPreview extends SurfaceView implements SurfaceHolder.Callback {

    private static String TAG = IDCameraPreview.class.getName();

    private Camera           camera;
    private IDAutoFocusManager mAutoFocusManager;
    private IDSensorControler  mSensorControler;
    private Context          mContext;
    private SurfaceHolder    mSurfaceHolder;

    public IDCameraPreview(Context context) {
        super(context);
        init(context);
    }

    public IDCameraPreview(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public IDCameraPreview(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public IDCameraPreview(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(Context context) {
        ...
        mSensorControler = IDSensorControler.getInstance(context.getApplicationContext());
    }

    public void surfaceCreated(SurfaceHolder holder) {
        camera = IDCameraUtils.openCamera();
        if (camera != null) {
            try {
    ...
    public void onStart() {
        ...
        if (mSensorControler != null) {
            ...
            mSensorControler.setCameraFocusListener(new IDSensorControler.CameraFocusListener() {
                @Override
                public void onFocus() {
                    focus();
                }
